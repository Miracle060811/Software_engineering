...
系统核心主链路可运行，课程验收证据已按源码、文档、DevOps、测试、管理和答辩六类归档。UC01—UC19 的证据基线由 [`docs/ci/use-case-test-matrix.json`](docs/ci/use-case-test-matrix.json) 与 [`docs/ci/test-quality-policy.json`](docs/ci/test-quality-policy.json) 管理；六微服务的 216 个 HTTP 端点（151 个公开、65 个内部）测试映射由 [`docs/ci/microservice-api-coverage.json`](docs/ci/microservice-api-coverage.json) 管理，不能把“代码已存在”直接视为“场景已被完整自动化覆盖”。
...
运营仪表盘通过内部接口同时汇总交通订单与 `local-service` 中的酒店、景点门票和跟团游订单；页面只展示面向管理员的状态说明，不暴露服务名、响应字段名或课程测试文案。
