...
| `local-service` | 8083 | 酒店、景点、目的地、评价、优惠券、本地游、本地生活 Outbox | 提供优惠券核销与仪表盘本地订单内部接口 |
...
| `ops-service` | 8086 | 敏感词、操作日志 | 通过各业务服务内部接口聚合管理列表、完整订单统计并下发审核、退款等命令 |
...
六个服务的 216 个 Controller 端点（151 个公开端点、65 个内部端点）均登记了正常、鉴权和参数边界 MockMvc 测试锚点；有跨服务依赖的端点另登记 503 失败测试。清单由代码自动发现并与测试源码互相校验，新增或删除路由后未同步测试会使门禁失败：
