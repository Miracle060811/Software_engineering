...
public class PostServiceImpl implements PostService {
    ...
    public List<Map<String, Object>> listPosts(int page, int size, String keyword) {
        ...
        // 只返回已发布且公开的帖子
        List<Map<String, Object>> posts = postMapper.selectPostsWithUser(
    ...
    private void fillAuthor(Post post) {
        ...
        UserSummary author = identityGateway.findUser(post.getUserId());
        if (author == null) {
            return;
        }
        post.setAuthorUsername(author.username());
        post.setAuthorNickname(author.nickname());
        post.setAuthorAvatar(author.avatar());
    }
    ...
    private boolean isFollowing(Long followerId, Long followeeId) {
        ...
        return identityGateway.isFollowing(followerId, followeeId);
    }
    ...
    public List<Map<String, Object>> getFollowingPosts(Long userId, int page, int size, String keyword) {
        // 获取用户关注的人列表
        List<Long> followeeIds = identityGateway.followingIds(userId);
        if (followeeIds.isEmpty()) return Collections.emptyList();

        PaginationSupport.Page pagination = PaginationSupport.normalize(page, size, MAX_PAGE_SIZE);
        List<Map<String, Object>> posts = postMapper.selectPostsByUserIds(
                followeeIds, pagination.offset(), pagination.size(), normalizeKeyword(keyword));
        enrichPosts(posts);
        return posts;
    }

    private void enrichPosts(List<Map<String, Object>> posts) {
        Set<Long> userIds = new HashSet<>();
        for (Map<String, Object> post : posts) {
            Object raw = post.get("userId");
            if (raw instanceof Number number) userIds.add(number.longValue());
        }
        Map<Long, UserSummary> users = identityGateway.findUsers(userIds);
        for (Map<String, Object> post : posts) {
            Object raw = post.get("userId");
            UserSummary user = raw instanceof Number number ? users.get(number.longValue()) : null;
            post.put("authorUsername", user == null ? "匿名用户" : user.username());
            post.put("authorNickname", user == null ? "匿名用户" : user.nickname());
            post.put("authorAvatar", user == null ? null : user.avatar());
        }
    }
