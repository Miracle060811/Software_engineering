...
本目录将六个业务服务部署到独立的 `travelmate-microservices` 命名空间，不修改现有 `deploy/k8s` 单体部署。

## 资源组成

| 类型 | 资源 | 副本与持久化 |
| --- | --- | --- |
| 业务服务 | `identity-service`、`traffic-service`、`local-service`、`ai-service`、`community-service`、`ops-service` | 初始 2 副本；HPA 最少 2、最多 6，CPU 目标 60% |
