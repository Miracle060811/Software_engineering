...
- **核心设计**：实现酒店多维度搜索过滤、酒店 / 景点详情展示、基础五星图文评价提交与列表展示。房务预订中的库存防超卖逻辑由 A 的订单库存管控模块统一设计；景点门票购买入口已在目的地流程中提供，核销二维码作为订单/凭证展示能力处理；商家回复、评价举报处理、酒店 / 景点后台维护由 E 的管理后台负责。

**3. AI 智能规划与 Agent 服务子系统（模块负责人：C 陈一鸿）**

- **功能职责**：负责平台所有与大语言模型相关的智能化场景以及全局消息触达。
- **核心设计**：统一封装并异步调用 DeepSeek API。行程规划模块强制使用 **JSON Output** 格式化返回结果，保障前端渲染的结构化；AI 客服 Agent 结合自然语言意图识别与 **Skills/内部工具调用**（如查询平台站内酒店数据），实现多轮智能问答。此外，负责统筹平台实时通知服务（站内信、红点未读逻辑），并在行程与门票场景中展示景点预订入口和订单/凭证信息。
...
- **核心设计**：搭建基于 **RBAC（基于角色的访问控制）** 的权限与安全架构；实现轻量数据可观测仪表盘（如实时订单大屏、本地访问量 QPS 监控、报错日志看板）；构建内容与安全审核防线（社区图文敏感词自动过滤 + 人工审核工作流）；提供大交通、酒店、景点基础数据的后台 CRUD 管理能力，以及商家回复、评价举报工单处理等运营侧功能。
...
- **处理**：
  ...
  2.  **数据库原子校验**：订单落库时，放弃重量级的行级悲观锁，**采用数据库原子更新语句**进行库存校验（例如：`UPDATE tm_hotel_room SET available_rooms = available_rooms - 1 WHERE id = ? AND available_rooms > 0`）。
...
- **处理**：
  ...
  3.  **房务预订 & 防超卖**：接口入口保留在酒店订单流程中，具体 Redis 预减、数据库原子扣减、订单状态流转由 TM-MOD-02 / A 模块统一实现，B 只提供酒店与房型基础数据支持。
  4.  **景点 / 本地玩乐**：提供景点列表与详情展示；门票购买入口通过 `/api/attraction/{id}/ticket` 提供，订单/凭证展示用于演示核销流程。
...
- **处理**：
  ...
  6.  **可观测大屏**：统计在线用户数、订单总量、今日 GMV、接口平均响应时长、错误日志列表、热门目的地 TOP10；数据来源为 `sys_log` 聚合 + Redis 实时计数，前端使用 ECharts 可视化展示。
  7.  **举报工单**：查看 `tm_review_report` 举报列表，支持驳回或删除被举报评价。
  8.  **敏感词维护**：动态添加 / 删除敏感词库，立即生效于发帖审核流程。
  
- **关键实体**：`sys_log`、`tm_coupon`、`tm_user_coupon`、`sys_sensitive_word`、`tm_review_report`。
...
| GET  | `/api/user/profile/{username}` | 查看用户主页 | 公开 |
...
| GET  | `/api/comment/list`        | 评论列表           | 公开   |
| POST | `/api/comment/add`         | 发表评论           | 需登录 |
| DELETE | `/api/comment/{id}`       | 删除评论           | 需登录 |
| POST | `/api/follow/{userId}`     | 关注/取关用户      | 需登录 |
| GET  | `/api/follow/fans/{userId}` | 粉丝列表          | 公开   |
| GET  | `/api/follow/following/{userId}` | 关注列表     | 公开   |
...
| `tm_traffic_order` | `id`, `order_no`, `user_id`, `order_type`(0 机票/1 火车票), `ticket_id`, `seat_type`, `passenger_name`, `passenger_id_card`, `amount`, `status`, `pay_time`, `create_time` | 大交通订单 |
...
| `tm_coupon`        | `id`, `name`, `description`, `discount_type`, `discount_value`, `min_amount`, `expire_date`, `stock`, `status` | 优惠券 |
...
| `tm_hotel_order`   | `id`, `order_no`, `user_id`, `hotel_id`, `room_id`, `hotel_name`, `room_type`, `check_in_date`, `check_out_date`, `nights`, `guest_name`, `guest_phone`, `amount`, `status`, `create_time` | 酒店订单 |
...
- **AI 使用附件**：单独整理 Prompt 模板、JSON Output 约束、Function Calling 工具定义、降级模板和 API Key 配置说明，增强大模型使用的可解释性。
