...
public interface TrainMapper extends BaseMapper<Train> {
    ...
    /** 归还一等座 */
    @Update("UPDATE tm_train SET first_class_seats = first_class_seats + 1 WHERE id = #{id}")
    int returnFirstClassSeat(@Param("id") Long id);

    /** 归还二等座 */
    @Update("UPDATE tm_train SET second_class_seats = second_class_seats + 1 WHERE id = #{id}")
    int returnSecondClassSeat(@Param("id") Long id);
}
