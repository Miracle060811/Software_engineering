...
public class TravelPlaceService {
    ...
    private VerifiedPlace findLocalCity(String input, String key) {
        ...
        if (!known && destinationMapper != null) {
            try {
                Long count = destinationMapper.selectCount(new LambdaQueryWrapper<Destination>()
                        .eq(Destination::getStatus, 1)
                        .and(wrapper -> wrapper.eq(Destination::getName, input)
                                .or().eq(Destination::getSlug, input)));
                known = count != null && count > 0;
            } catch (Exception ex) {
                log.debug("读取本地目的地库失败: {}", ex.getMessage());
            }
        }
        return known ? new VerifiedPlace(input, canonicalLocalName(input), input, "", Double.NaN, Double.NaN,
                "TravelMate 本地目的地库") : null;
    }
