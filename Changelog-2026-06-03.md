...
## 追加记录：火车票城市级查询与候补交互增强

### feat · New Feature

- **城市名自动展开为站点组合**: 火车票搜索从精确站点匹配扩展为城市/站点解析，用户输入“北京”“南昌”“泉州”等城市名时，会按内置城市站点优先级展开为多个 12306 可识别站点组合，再逐组读取公开余票页面；本地数据库兜底查询也同步支持城市内站点集合匹配，减少“北京查不到、北京西能查到”这类日常使用落差。
- **按目标数量聚合 12306 结果**: 实时读取逻辑从“第一组站点组合有结果就返回”调整为“默认收集 10 条去重车次，未满则继续查下一个站点组合，候选组合耗尽则返回已有结果”；后端按发车时间排序后返回，避免北京-天津等高密度短途线路只能显示一两趟。
- **读取更多分页能力**: `/api/train/search` 新增 `offset` 与 `limit` 参数，默认读取 10 条；前端火车票搜索页新增“读取更多”按钮，点击后以当前已展示数量作为 offset 继续读取下一批 10 条并追加展示，避免一次性读取过多 12306 页面组合导致等待时间过长。
- **无票车次可展示并候补**: 搜索结果不再因为一等座/二等座余票为 0 就过滤整趟车；若车次存在但当前网站可预订席别无票，前端展示余票/候补状态并提供“提交候补”按钮，用于证明查询链路正常、只是当前无可售票。
- **站内候补申请表**: 新增 `tm_train_waitlist`、`TrainWaitlist`、`TrainWaitlistService` 和 `/api/train/waitlist`，用户登录后可为无票车次提交站内候补申请，记录车次、区间、出发时间、席别、票数和乘车人信息。
- **查询区间缓存唯一键调整**: `tm_train` 唯一键由 `train_no + departure_time` 调整为 `train_no + departure_station + arrival_station + departure_time`，支持同一车次按用户实际查询区间缓存，例如只保存“泉州 -> 上海”的结果而不强制保存完整经停表。

### verification · 验证

- 已使用本地 MySQL 密码完成最小迁移：创建 `tm_train_waitlist`，并将 `tm_train` 唯一索引调整为 `uk_train_segment_deptime`。
- 后端 `.\mvnw.cmd -q -DskipTests compile` 通过。
- 前端 `npm run build` 通过，仅保留 Vite/Rollup 既有的大 chunk 体积提示与依赖注释提示。
- 尝试使用 Codex 内置浏览器打开本地 Vite 页面时被客户端拦截为 `ERR_BLOCKED_BY_CLIENT`，因此本轮浏览器冒烟未完成；功能验证以编译、构建和数据库结构检查为准。
