...
const listPath = path.join(root, "02_docs", "业务场景清单与用例说明.md");
const designPath = path.join(root, "02_docs", "5组-软件详细设计说明.md");
const requirementsPath = path.join(root, "02_docs", "5组-软件需求规格说明.md");
const overviewPath = path.join(root, "02_docs", "软件概要设计说明书.md");
const traceabilityPath = path.join(root, "02_docs", "需求设计代码测试追溯表.md");
const sysDiagramPath = path.join(root, "02_docs", "需求规格说明");
const sysSourcePath = path.join(sysDiagramPath, "source");
const compDiagramPath = path.join(root, "02_docs", "概要设计说明");
const compSourcePath = path.join(compDiagramPath, "source");
const diagramPath = path.join(root, "02_docs", "详细设计说明");
