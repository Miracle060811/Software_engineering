...
def draw_use_cases():
    ...
    groups = [
        ("游客/用户", ["UC01 注册登录与账户安全", "UC02 查询并预订航班", "UC03 查询火车票/候补", "UC04 交通订单履约退款", "UC05 搜索酒店并订房", "UC06 酒店订单履约退款", "UC07 景点浏览与购票", "UC08 本地游产品预订", "UC10 优惠券领取使用", "UC11 AI 行程生成保存", "UC12 AI 客服对话", "UC13 通知与私信", "UC14 游记发布审核", "UC15 点赞收藏评论", "UC16 常用旅客管理", "UC17 用户主页与关注"]),
...
def main():
    ...
    sequences = [
        ...
        (43, "UC08", "浏览一日游周边游产品", ["用户", "AttractionList/MyOrders.vue", "TourProductController", "TourProductServiceImpl", "Product/Step Mapper", "Schedule/Order Mapper"], [(0,1,"选择产品、班期、人数和联系人","call"),(1,2,"POST /api/tour/orders + Idempotency-Key","call"),(2,3,"createBooking(userId,dto)","call"),(3,4,"查询产品与行程步骤","call"),(3,5,"校验班期并条件扣减库存","call"),(5,3,"返回更新行数；不足则失败","return"),(3,5,"插入订单或返回幂等原订单","call"),(5,3,"返回本人订单","return"),(3,2,"TourOrder / 业务错误","return")], "价格由服务端复核，金额按班期单价×人数计算；失败回滚且不产生脏订单。"),
