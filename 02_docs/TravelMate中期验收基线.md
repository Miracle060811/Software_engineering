...
TravelMate 以单个 `backend` Maven 工程作为现有功能回归基线；在中期设计冻结后，新增 `microservices` 多模块工程，第一批 `SVC-IDENTITY`、`SVC-TRAFFIC`、`SVC-LOCAL`、`SVC-AI` 可独立构建、启动和制作镜像。中期时点采用渐进式迁移：完成四服务分库 DDL、本地 Compose、事务 Outbox 写入/重试投递、AI 通知幂等消费和数据迁移工具。最终阶段扩展为六服务，并完成真实数据迁移、Kubernetes 部署、HPA 和故障恢复证据归档。
...
中期三项设计交付已经完成，当时形成四个独立可执行 JAR 和四个 Dockerfile。最终实现采用六服务、分库脚本与容器化流水线；课程架构不采用 API Gateway、注册中心或配置中心。
...
| 真实数据切换验收 | 最终阶段已完成 | 2026-09-02 在 MySQL 8.4 隔离环境完成 31/31 张表迁移一致性校验，证据见 `../04_tests/migration/results/data-migration-2026-09-02/` |
...
1. 先说明历史时点：单体作为回归基线保留，中期完成首批四服务；最终阶段已形成六服务、真实数据迁移和 Kubernetes 部署证据。
...
1. 抽取 `Result<T>`、鉴权 Claims 等公共契约，公共模块不包含业务 Entity/Mapper。
2. 第一批拆出 SVC-IDENTITY、SVC-TRAFFIC、SVC-LOCAL，并建立独立 `pom.xml`、测试入口、Dockerfile 和数据库 Schema。
3. 将原 Java 进程内跨域调用替换为内部 REST 或事件，加入超时、幂等、重试与补偿测试。
4. 拆分 AI、社区与运营服务，形成六服务独立构建、测试、镜像和 Kubernetes 部署流水线。
5. 通过端到端回归、数据迁移、HPA、故障隔离与限定回滚实验完成验收。
